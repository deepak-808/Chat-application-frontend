import { FormControl, InputLabel, MenuItem, Select, TableHead, TableSortLabel, Tooltip, Typography } from '@mui/material';
...
function createData(_id, name, carID, status, carImage, createdAt, updatedAt, isDeleted) {
  return { _id, name, carID, status, carImage, createdAt, updatedAt, isDeleted };
}
...
const CarModels = () => {
  ...
  useEffect(() => {
    const getCarModelData = async () => {
      try {
        if (carBrand !== '') {
          const response = await axios.get(`${API_URL}car/carsModel?carID=${carBrand}`);
  ...
  return (
    <Box sx={{ width: '100%', p: 2 }}>
      <Box sx={{ color: 'var(--primary)', marginBottom: 8 }} className="d-flex">
        <Typography variant="h5">Car Models</Typography>
        <Typography onClick={() => navigate('/car/model/add')} className='ms-auto cursor-pointer'>Add Car Model <IoIosAddCircleOutline size='20px'/></Typography>
      </Box>
      <FormControl className='w-50 mb-4'>
        <InputLabel id="demo-simple-select-label">Car Models</InputLabel>
        <Select
                  ...
                  .map((row, index) => {
                    return (
                      <TableRow key={row.name}>
                        ...
                        <TableCell align='right' style={{ width: 100 }}>{moment(row.createdAt).format("MMM Do YY")}</TableCell>
                        <TableCell style={{ width: 100 }} align="right">
